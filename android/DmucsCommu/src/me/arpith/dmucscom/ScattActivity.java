...
public class ScattActivity extends ScatterFriend {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		String[] proj = new String[] { BaseColumns._ID, Constant.AUTHORID,
				Constant.AUTHOR, Constant.MES, Constant.DATETIME };

		showNumofMem();

		Cursor c = managedQuery(Constant.TWEET_CONTENT_URI, proj, null, null,
				null);
		SimpleCursorAdapter adapter = new SimpleCursorAdapter(this,
				R.layout.tweetitem, c, new String[] { Constant.AUTHOR,
						Constant.DATETIME, Constant.MES }, new int[] {
						R.id.tweetAuthor, R.id.tweetTimeStamp, R.id.tweetMes });
		tweetList.setAdapter(adapter);

		Cursor tweets = getContentResolver().query(Constant.TWEET_CONTENT_URI,
				proj, null, null, null);
		if (tweets.getCount() == 0)
			insertToDB("Disaster Recovery",
					"You may now start chatting with people around you..");
	}
	...
	public void writeTweet(View v) {
		...
		if (!myTweet.equals("")) {
			...
			if (insert)
				writeTweetView.setText("");
		}
		try {
			// All messages to be sent out must be separated with "|" between
			// units
			...
			beddernetService.sendMulticast(devices, null, myTweet.getBytes(),
				...
				while (!abort) {
					try {
						if (beddernetService != null)
							devices = beddernetService
									.getDevices(applicationIdentifier);
					...
					runOnUiThread(new Runnable() {
						...
						public void run() {
							...
							writeTweetView.setHint("Number of devices: "
									+ devices.length);
						}
					});
					try {
